...
func newTCPLifecycleEnricher(includeExtendedFields bool) *tcpLifecycleEnricher {
	return newTCPLifecycleEnricherWithProcessCache(includeExtendedFields, nil)
}

func newTCPLifecycleEnricherWithProcessCache(includeExtendedFields bool, processes *processCache) *tcpLifecycleEnricher {
...
func newTCPLifecycleEnricherWithLookups(includeExtendedFields bool, maxEntries int, lookups tcpLifecycleEnrichmentLookups) *tcpLifecycleEnricher {
	if maxEntries < 0 {
		maxEntries = 0
	}
...
func (enricher *tcpLifecycleEnricher) Enrich(payload tcpLifecycleEventPayload) tcpLifecycleEventPayload {
	...
	payload.ProcessStartTimeTicks = cloneUint64Pointer(enrichment.ProcessStartTimeTicks)
	...
	case tcpLifecycleEventTypeConnectFailed, tcpLifecycleEventTypeClosed:
...
func (enricher *tcpLifecycleEnricher) lookup(payload tcpLifecycleEventPayload) tcpLifecycleEnrichment {
	pid := int(payload.PID)
	enrichment := tcpLifecycleEnrichment{}
	trackedProcess := false

	if enricher.processes != nil {
		snapshot, found, tracked := enricher.processes.Lookup(payload.PID, payload.UID, payload.Comm, payload.KernelTimestampNS)
		...
		if found {
			...
			applyProcessContext(&enrichment, snapshot.Context, uint64PointerIfNonZero(snapshot.CgroupID))
...
func applyProcessContext(enrichment *tcpLifecycleEnrichment, context processContextSnapshot, cgroupID *uint64) {
	if context.StartTimeTicks != 0 {
		enrichment.ProcessStartTimeTicks = uint64Pointer(context.StartTimeTicks)
	}
	if context.ParentPID != 0 {
		parent := &tcpLifecycleProcessParentPayload{PID: context.ParentPID}
	...
	if cgroupID != nil || context.CgroupPath != "" {
		enrichment.Cgroup = &tcpLifecycleCgroupPayload{ID: cloneUint64Pointer(cgroupID), Path: context.CgroupPath}
	}
	...
	if context.Container != nil {
		enrichment.Container = &tcpLifecycleContainerPayload{Runtime: context.Container.Runtime, ID: context.Container.ID}
	}
}
...
func lookupTCPLifecycleUsername(uid uint32) string {
	...
	if userInfo, err := user.LookupId(uidText); err == nil { return userInfo.Username }
	return uidText
}

func lookupTCPLifecycleASN(payload tcpLifecycleEventPayload) *tcpLifecycleASNPayload {
	if payload.Remote.IP == nil { return nil }
	if payload.Remote.IP.To4() != nil {
		info := as.GetASInfoIPv4(payload.Remote.IP)
		if info.AsNumber == 0 { return nil }
		return &tcpLifecycleASNPayload{Number: info.AsNumber, Name: info.Name}
	}
...
func lookupTCPLifecycleDNS(payload tcpLifecycleEventPayload) *dnsCorrelationPayload {
	return lookupDNSCorrelation(payload.Remote.IP, payload.PID, payload.ObservedAt)
}

func cloneTCPLifecycleParent(value *tcpLifecycleProcessParentPayload) *tcpLifecycleProcessParentPayload {
	if value == nil { return nil }
	return &tcpLifecycleProcessParentPayload{PID: value.PID, StartTimeTicks: cloneUint64Pointer(value.StartTimeTicks)}
}
func cloneTCPLifecycleCgroup(value *tcpLifecycleCgroupPayload) *tcpLifecycleCgroupPayload {
	if value == nil { return nil }
	return &tcpLifecycleCgroupPayload{ID: cloneUint64Pointer(value.ID), Path: value.Path}
}
func cloneTCPLifecycleNamespaces(value *tcpLifecycleNamespacesPayload) *tcpLifecycleNamespacesPayload {
	if value == nil { return nil }
	copied := *value
	return &copied
}
func cloneTCPLifecycleContainer(value *tcpLifecycleContainerPayload) *tcpLifecycleContainerPayload {
	if value == nil { return nil }
	copied := *value
	return &copied
}
func cloneTCPLifecycleASN(value *tcpLifecycleASNPayload) *tcpLifecycleASNPayload {
	if value == nil { return nil }
	copied := *value
	return &copied
}
